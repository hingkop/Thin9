...
def panoptic(args: argparse.Namespace) -> None:
    print(args)
    ...
    data_path = 'output_panoptic/6/sam_mask_label/sam_mask_label.txt'  # 파일 경로를 적절하게 수정해주세요
...
#     print(type(category_counts)) # pd.Series
    mask_label =list(category_counts.index)
    mask_label.remove('background') ###
    logger.info(mask_label)

if __name__ == "__main__":
    args = parser.parse_args()
    command_line_args = ['--img_path', 'images/8.jpeg', '--output', 'output_panoptic']
